# Local Occamy M3 Freeze

This document is the branch-local source of truth for the reduced Occamy path
on `occamy-minimal-bootstrap`.
...
- trace-backed evidence for the current M3 freeze
...
The current M3 freeze command is:

```bash
./scripts/run-local-occamy-openmp-replay-bridge.sh --all
```

Expected M3 evidence:
...
- The fake driver can model the subset of `/dev/occamydev--1` needed for M3
  smoke testing.
...
qemu-user       qemu-riscv64 for M3 OpenMP smoke tests
```
...
Plain M3 runtime smoke, expected to stop at missing real device endpoint:
...
Run the real Snitch-side mailbox-runtime smoke in Verilator:
...
- M3 build fixes in the OpenMP/common, libhero, and LLVM support paths
...
That is expected because fake completion does not execute target code.  The M3
