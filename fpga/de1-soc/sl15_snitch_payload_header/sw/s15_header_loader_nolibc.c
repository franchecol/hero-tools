...
static int read_payload_image(struct payload_desc *desc) {
    ...
    if (fd < 0) {
        write_all(STDERR_FD, "open SL15 payload image failed\n");
        return 1;
    }
    ...
    while (total < S15_MAX_IMAGE_BYTES) {
        ...
        if (n < 0) {
            write_all(STDERR_FD, "read SL15 payload image failed\n");
    ...
    if (total == S15_MAX_IMAGE_BYTES) {
        ...
        if (n > 0) {
            write_all(STDERR_FD, "SL15 payload image too large\n");
            syscall1(SYS_CLOSE, fd);
            return 1;
        }
    }
    ...
    if (total < (S15_HEADER_WORDS * 4u)) {
        write_all(STDERR_FD, "SL15 payload image is shorter than header\n");
        return 1;
    }
    if ((total & 3u) != 0) {
        write_all(STDERR_FD, "SL15 payload image is not word-aligned\n");
        return 1;
    }
    ...
    if (magic != S15_MAGIC || version != S15_VERSION || header_words != S15_HEADER_WORDS || flags != 0u) {
        write_all(STDERR_FD, "invalid SL15 payload header\n");
        return 1;
    }
    if (desc->entry_word != 0u) {
        write_all(STDERR_FD, "unsupported SL15 entry word\n");
        return 1;
    }
    if (desc->payload_word_count == 0u || desc->payload_word_count > S15_MAX_PAYLOAD_WORDS) {
        write_all(STDERR_FD, "invalid SL15 payload word count\n");
        return 1;
    }
    if ((S15_HEADER_WORDS + desc->payload_word_count) != (u32)(total / 4u)) {
        write_all(STDERR_FD, "SL15 image size does not match header word count\n");
        return 1;
    }
    if (((desc->arg0 + desc->arg1) & 0xffffffffu) != desc->expected) {
        write_all(STDERR_FD, "SL15 expected value does not match args\n");
        return 1;
    }
    ...
    if (checksum != desc->checksum) {
        write_all(STDERR_FD, "SL15 payload checksum mismatch\n");
        return 1;
    }
