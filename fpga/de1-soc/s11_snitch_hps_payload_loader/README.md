# Snitch-Lite S11: HPS Payload Loader

S11 continues after S10.

S10 proved that ARM Linux can pass input data to Snitch-Lite. S11 adds the
...
`sw/payload_sum.S`. At runtime it writes that image into the S11 payload window.
...
S11 clocks the HPS bridge-facing Qsys subsystem and Snitch-Lite core at 25 MHz
...
Final educational version:
  ...
  25 MHz S11 Qsys/Snitch-Lite subsystem
...
./scripts/serial_transfer.py send \
  ../s11_snitch_hps_payload_loader/build/s11_mmio_nolibc \
  /tmp/s11_mmio_nolibc
```
...
Build checks:
  ...
  sv2v S11 translation:                 pass
  Yosys S11 probe:                      pass
