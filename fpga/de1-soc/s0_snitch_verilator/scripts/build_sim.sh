...
export CXXFLAGS="${S0_HOST_CXXFLAGS:--include cstdint}"
export VERILATOR_ROOT="${S0_VERILATOR_ROOT:-/usr/share/verilator}"
export VLT_ROOT="${VERILATOR_ROOT}"

S0_DEFAULT_VLT_FLAGS=(
    --no-timing
    --no-assert
    -O0
    -fno-dfg
...
S0_DEFAULT_VLT_FLAGS_TEXT="${S0_DEFAULT_VLT_FLAGS[*]}"
...
make -C "${SNITCH_TARGET}" \
    ...
    VLT_FLAGS="${S0_VLT_FLAGS:-${S0_DEFAULT_VLT_FLAGS_TEXT}}" \
