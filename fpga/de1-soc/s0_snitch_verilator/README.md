# Snitch-Lite S0: Verilator Bring-Up

S0 starts the real-Snitch path without using the DE1-SoC ARM/Linux side.
...
```bash
cd /home/ftv/builds/hero-tools/fpga/de1-soc/s0_snitch_verilator
...
```text
S0 bare-metal Snitch simulation passed
```
...
tiny config does not implement yet. That is useful later, but it is not the S0
pass criterion.
...
After S0 passes, the next steps are:

```text
S1: add a tiny MMIO location to the simulated Snitch system
S2: make the bare-metal program write that MMIO location
S3: port the reduced Snitch system wrapper to Quartus
S4: connect that MMIO location to LEDR/HEX/UART on DE1-SoC
```
