...
"${S5_DIR}/scripts/build_sw_rom.sh"

SV2V_LOG="${GENERATED_DIR}/sv2v_s5.log"
YOSYS_LOG="${GENERATED_DIR}/yosys_s5.log"
OUT_V="${GENERATED_DIR}/snitch_generated_rom_mmio_led.v"
...
SOURCES=(
    ...
    "${FILTERED_SNITCH}"
    "${S5_DIR}/rtl/de1_s5_snitch_generated_rom_mmio_led.sv"
)
...
if [[ "${sv2v_status}" -ne 0 ]]; then
    echo "sv2v SL5 failed with exit code ${sv2v_status}"
...
if [[ "${yosys_status}" -ne 0 ]]; then
    echo "yosys SL5 failed with exit code ${yosys_status}"
