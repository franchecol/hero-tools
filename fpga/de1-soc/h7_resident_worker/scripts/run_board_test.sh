...
de1_dir=$(cd -- "${h7_dir}/.." && pwd)
h0_dir="${de1_dir}/h0_5_safe_host_control"
h6_dir="${de1_dir}/h6_linux_job_driver"
...
"${transfer}" --tty "${tty}" send "${h6_dir}/build/h6_client_nolibc" /tmp/h7_client_nolibc
"${transfer}" --tty "${tty}" send "${h7_dir}/build/resident_job.bin" /tmp/h6_payload.bin

export H7_SERIAL_RUN="${d3_dir}/scripts/serial_run.py" H7_TTY="${tty}"
...
commands = [
    sudo("rmmod snitch_lite_irq 2>/dev/null || true; rmmod snitch_job 2>/dev/null || true; "
         "insmod /tmp/snitch_job.ko gic_spi=40 resident_mode=1; "
    ...
    "chmod +x /tmp/h7_client_nolibc; /tmp/h7_client_nolibc",
    sudo("rmmod snitch_job"),
]
subprocess.run(
    [os.environ["H7_SERIAL_RUN"], "--tty", os.environ["H7_TTY"],
     "--timeout", "60", "--stop-on-error", "--require", "H7_RESIDENT_WORKER_PASS",
     "--require", "H6_LINUX_JOB_DRIVER_PASS"],
