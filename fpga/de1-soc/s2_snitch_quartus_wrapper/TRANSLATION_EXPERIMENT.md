# S2.1 Translation Experiment Result

This note records the experiment that tried to make the S2 Snitch cluster
Quartus-compatible by translating the generated SystemVerilog before Quartus
sees it.

## Goal

S2 proved that the Quartus project packaging path works:
...
The input file list came from the S2 Bender export:
...
```text
sv2v + Bender include dirs + Bender defines + all 312 files + S2 wrapper
```
...
2. It would be hard to trust.
   ...
   testing the same RTL structure that S0/S1 simulated.
...
The goal of S3 is not yet to run a complete program on the DE1-SoC board. The
...
If S3 succeeds, then the next step is to add real ROM/RAM/MMIO behavior around
that core.

## Follow-Up Status

S3 was created after this decision:

```text
fpga/de1-soc/s3_snitch_core_only_probe/
```

S3 validates the pivot:
