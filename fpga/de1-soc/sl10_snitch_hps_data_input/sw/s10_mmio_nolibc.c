...
#define REG_RAM1        0x2cu
#define REG_RAM2        0x30u
...
static int run_one(volatile unsigned char *regs, const char *tag, u32 arg0, u32 arg1) {
    ...
    if (result != expected || ram0 != arg0 || ram1 != arg1 || ram2 != expected) {
        write_all(STDERR_FD, "unexpected Snitch data result\n");
        return 1;
    }
...
static int run_test(void) {
    ...
    if (id != 0x53100001u) {
        write_all(STDERR_FD, "unexpected ID; check SL10 bitstream, bridge enable, and address\n");
        syscall3(SYS_MUNMAP, mapped, MAP_LEN, 0);
        syscall1(SYS_CLOSE, fd);
        return 1;
    }

    write_hex32("ROM_WORDS  ", read_reg(regs, REG_ROM_WORDS));
