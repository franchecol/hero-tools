# Snitch-Lite SL12: HPS File Payload Loader

SL12 continues after SL11.

SL11 proved that ARM Linux can write a tiny RISC-V instruction payload into FPGA
instruction memory. SL12 keeps the same SL11 FPGA bitstream and moves the payload
image out of the ARM tester binary:

```text
ARM Linux filesystem
  -> /tmp/s12_payload.bin
  -> SL12 ARM loader reads the file
  -> /dev/mem
  -> lightweight HPS-to-FPGA bridge at 0xff200000
  -> SL11 payload instruction-memory window
  -> CONTROL.start
...
SL12 does not add new RTL. Program the SL11 bitstream first:
...
```bash
cd /home/ftv/builds/hero-tools/fpga/de1-soc/sl12_snitch_hps_file_loader
./scripts/build.sh
```
...
It is the same tiny RV32E-style payload shape as SL11: load RAM0/RAM1, add them,
store to RAM2, write done/result MMIO, then park.

## Program And Run

With the SL11 bitstream already programmed, transfer and run SL12:
...
./scripts/serial_transfer.py send \
  ../sl12_snitch_hps_file_loader/build/s12_file_loader_nolibc \
  /tmp/s12_file_loader_nolibc
./scripts/serial_transfer.py send \
  ../sl12_snitch_hps_file_loader/generated/sw/s12_payload.bin \
  /tmp/s12_payload.bin
```
...
Verified on the local DE1-SoC board using the SL11 bitstream already loaded by
USB-Blaster/JTAG.
...
The same SL11 bitstream can run file-loaded Snitch code with different inputs.
```
