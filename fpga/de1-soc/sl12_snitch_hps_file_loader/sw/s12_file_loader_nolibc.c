...
#define S12_PAYLOAD_PATH "/tmp/s12_payload.bin"
#define S12_MAX_PAYLOAD_WORDS 16u
#define S12_MAX_PAYLOAD_BYTES (S12_MAX_PAYLOAD_WORDS * 4u)
...
static u8 payload_bytes[S12_MAX_PAYLOAD_BYTES];
static u32 payload_words[S12_MAX_PAYLOAD_WORDS];
...
static int read_payload_file(u32 *word_count_out) {
    int fd = (int)syscall3(SYS_OPEN, (long)S12_PAYLOAD_PATH, O_RDONLY, 0);
    if (fd < 0) {
    ...
    usize total = 0;
    while (total < S12_MAX_PAYLOAD_BYTES) {
        long n = syscall3(
            SYS_READ,
            fd,
            (long)(payload_bytes + total),
            (long)(S12_MAX_PAYLOAD_BYTES - total));
        if (n < 0) {
    ...
    if (total == S12_MAX_PAYLOAD_BYTES) {
        u8 extra;
...
static int run_test(void) {
    ...
    if (id != 0x53110001u) {
        write_all(STDERR_FD, "unexpected ID; program the SL11 bitstream first\n");
