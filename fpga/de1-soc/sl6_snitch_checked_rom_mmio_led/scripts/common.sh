...
S6_DIR=$(cd -- "${SCRIPT_DIR}/.." && pwd)
REPO_ROOT=$(git -C "${S6_DIR}" rev-parse --show-toplevel)
...
GENERATED_DIR="${S6_DIR}/generated"
SW_BUILD_DIR="${GENERATED_DIR}/sw"
PROJECT="de1_s6_snitch_checked_rom_mmio_led"
SV2V="${SV2V:-sv2v}"
YOSYS="${YOSYS:-yosys}"
RISCV_PREFIX="${RISCV_PREFIX:-riscv64-unknown-elf}"
S6_SW_SRC="${S6_SW_SRC:-${S6_DIR}/sw/led_mmio.S}"
S6_ROM_MAX_WORDS="${S6_ROM_MAX_WORDS:-64}"
...
echo "S6_SW_SRC         = ${S6_SW_SRC}"
echo "S6_ROM_MAX_WORDS  = ${S6_ROM_MAX_WORDS}"
