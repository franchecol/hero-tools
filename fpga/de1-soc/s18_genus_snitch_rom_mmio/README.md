# S18: Genus Snitch Core Executes ROM Software

S18 moves the S17 Genus-to-Quartus path from structural activity to
...
## Why This Is Not A Repeat Of S5

S5 proved the same basic ROM/MMIO software behavior through the local reduced
`sv2v` Snitch path.  S18 keeps the software action intentionally familiar but
changes the core implementation boundary:

```text
S5:  upstream Snitch -> sv2v -> Quartus
S18: upstream Snitch -> Genus syn_generic -> sanitized names -> Quartus
```

Passing S18 therefore validates software-visible behavior across the new
external-synthesis handoff proposed after S16.

## Build And Program

The S17 generic netlist must already be present and checksum-verified under
`../s17_genus_snitch_core/build/`.  If necessary, retrieve it with:

```bash
../s17_genus_snitch_core/scripts/fetch_generic_netlist.sh
```
...
cannot reach, explaining the small 82-ALM result.  S17's dynamic-instruction
build remains the representative reduced-core resource measurement at 1216
ALMs and 988 registers.  S18 answers a different question: whether deterministic
...
The user physically confirmed `LEDR[9:0] = 0x355`, completing S18.  A later
