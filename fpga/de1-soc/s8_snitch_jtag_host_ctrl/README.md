# Snitch-Lite S8: JTAG Host Control

S8 adds a temporary host-control path around the S7 Snitch-Lite RAM check.
...
JTAG-to-Avalon master
      │
      ▼
S8 Avalon-MM control/status registers
      │
      ▼
...
So S8 is the last useful pre-microSD step: it proves the register contract
...
```bash
cd /home/ftv/builds/hero-tools/fpga/de1-soc/s8_snitch_jtag_host_ctrl
...
If the build and smoke test pass, S8 proves:
...
The Snitch core still performs the S7 store/load/compare sequence internally.
```
