...
S8_DIR=$(cd -- "${SCRIPT_DIR}/.." && pwd)
REPO_ROOT=$(git -C "${S8_DIR}" rev-parse --show-toplevel)
...
GENERATED_DIR="${S8_DIR}/generated"
SW_BUILD_DIR="${GENERATED_DIR}/sw"
PROJECT="de1_s8_snitch_jtag_host_ctrl"
QSYS_SYSTEM="s8_jtag_system"
SV2V="${SV2V:-sv2v}"
YOSYS="${YOSYS:-yosys}"
RISCV_PREFIX="${RISCV_PREFIX:-riscv64-unknown-elf}"
S8_SW_SRC="${S8_SW_SRC:-${S8_DIR}/sw/ram_check.S}"
S8_ROM_MAX_WORDS="${S8_ROM_MAX_WORDS:-64}"
...
echo "S8_SW_SRC         = ${S8_SW_SRC}"
echo "S8_ROM_MAX_WORDS  = ${S8_ROM_MAX_WORDS}"
