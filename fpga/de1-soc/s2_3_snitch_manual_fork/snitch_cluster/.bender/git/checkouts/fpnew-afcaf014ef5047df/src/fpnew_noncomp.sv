...
module fpnew_noncomp #(
  ...
  for (genvar i = 0; i < NUM_OUT_REGS; i++) begin : gen_output_pipeline
    ...
    `FFL(out_pipe_extension_bit_q[i+1], out_pipe_extension_bit_q[i], reg_ena, '0, clk_i, rst_ni)
    `FFL(out_pipe_class_mask_q[i+1], out_pipe_class_mask_q[i], reg_ena, fpnew_pkg::QNAN, clk_i, rst_ni)
    `FFL(out_pipe_is_class_q[i+1], out_pipe_is_class_q[i], reg_ena, '0, clk_i, rst_ni)
