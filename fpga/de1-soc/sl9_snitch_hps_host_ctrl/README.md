# Snitch-Lite SL9: HPS Host Control

SL9 is the direct continuation after D3.
...
and replaces the D3 toy register accelerator with the SL8 Snitch-Lite
...
├────────┼─────────────┼────────┼──────────────────────────────────────┤
│ 0x00   │ ID          │ R      │ 0x53380001, inherited SL8 block ID     │
...
```bash
cd /home/ftv/builds/hero-tools/fpga/de1-soc/sl9_snitch_hps_host_ctrl
./scripts/build.sh
./scripts/build_arm_tester.sh
```

The build script reuses the SL8 Snitch-Lite RTL generation flow, copies the
...
./scripts/serial_transfer.py send \
  ../sl9_snitch_hps_host_ctrl/build/s9_mmio_nolibc \
  /tmp/s9_mmio_nolibc
```
...
The Snitch core still performs the SL7/SL8 store/load/compare payload internally.
```
