package s13_snitch_irq_payload_core_pkg;
...
module s13_snitch_irq_payload_core (
    input  logic        clk,
    input  logic        reset,
    ...
    import snitch_pkg::*;
    import s13_snitch_irq_payload_core_pkg::*;

    localparam logic [31:0] ID_VALUE = 32'h5313_0001;
    ...
    localparam logic [7:0] REG_IRQ_ENABLE    = 8'h0e;
    localparam logic [7:0] REG_IRQ_PENDING   = 8'h0f;
    ...
    assign acc_resp = '0;
    assign ptw_pte = '0;
    assign irq_line = irq_enable_q && irq_pending_q;
    assign irq = irq_line;
            ...
            if (avs_write && !busy) begin
                unique case (avs_address)
                    ...
                    REG_IRQ_ENABLE: begin
                        if (avs_byteenable[0]) irq_enable_q <= avs_writedata[0];
                    end
                    REG_IRQ_PENDING: begin
                        if (avs_byteenable[0] && avs_writedata[0]) irq_pending_q <= 1'b0;
                    end
                    default: begin
                    end
                endcase
    ...
    always_comb begin
        unique case (avs_address)
            ...
            REG_IRQ_ENABLE:    avs_readdata = {31'd0, irq_enable_q};
            REG_IRQ_PENDING:   avs_readdata = {29'd0, irq_line, irq_enable_q, irq_pending_q};
            default: begin
                if (payload_access) begin
