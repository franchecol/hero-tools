...
SV2V_LOG="${GENERATED_DIR}/sv2v_s13.log"
YOSYS_LOG="${GENERATED_DIR}/yosys_s13.log"
...
SOURCES=(
    ...
    "${FILTERED_SNITCH}"
    "${S13_DIR}/rtl/s13_snitch_irq_payload_core.sv"
)
...
if [[ "${sv2v_status}" -ne 0 ]]; then
    echo "sv2v SL13 failed with exit code ${sv2v_status}"
...
echo "sv2v SL13 passed"
echo "Output: ${OUT_V}"
echo "Log: ${SV2V_LOG}"

set +e
"${YOSYS}" -q -p "read_verilog ${OUT_V}; hierarchy -top s13_snitch_irq_payload_core; proc; check" \
    > "${YOSYS_LOG}" 2>&1
yosys_status=$?
set -e

if [[ "${yosys_status}" -ne 0 ]]; then
    echo "yosys SL13 failed with exit code ${yosys_status}"
