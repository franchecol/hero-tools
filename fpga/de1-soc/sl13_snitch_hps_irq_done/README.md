# Snitch-Lite SL13: HPS Done IRQ

SL13 continues after SL12.

SL12 proved that ARM Linux can keep the ARM host executable and RISC-V/Snitch
payload image as separate files. SL13 keeps that file-loaded payload flow and
...
```text
SL13 verifies IRQ production and clear at the FPGA/MMIO level.
SL13 does not yet install a Linux kernel/UIO driver that blocks on the HPS IRQ.
```
...
./scripts/serial_transfer.py send \
  ../sl13_snitch_hps_irq_done/build/s13_irq_nolibc \
  /tmp/s13_irq_nolibc
./scripts/serial_transfer.py send \
  ../sl13_snitch_hps_irq_done/generated/sw/s13_payload.bin \
  /tmp/s13_payload.bin
```
...
Build checks:
  ...
  sv2v SL13 translation:                  pass
  Yosys SL13 probe:                       pass
