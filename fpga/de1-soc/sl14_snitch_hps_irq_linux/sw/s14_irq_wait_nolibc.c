...
#define S14_PAYLOAD_PATH "/tmp/s14_payload.bin"
#define S14_IRQ_DEV      "/dev/snitch_lite_irq"
#define S14_MAX_PAYLOAD_WORDS 16u
#define S14_MAX_PAYLOAD_BYTES (S14_MAX_PAYLOAD_WORDS * 4u)
...
static u8 payload_bytes[S14_MAX_PAYLOAD_BYTES];
static u32 payload_words[S14_MAX_PAYLOAD_WORDS];
...
static int read_payload_file(u32 *word_count_out) {
    int fd = (int)syscall3(SYS_OPEN, (long)S14_PAYLOAD_PATH, O_RDONLY, 0);
    if (fd < 0) {
    ...
    usize total = 0;
    while (total < S14_MAX_PAYLOAD_BYTES) {
        long n = syscall3(
            SYS_READ,
            fd,
            (long)(payload_bytes + total),
            (long)(S14_MAX_PAYLOAD_BYTES - total));
        if (n < 0) {
...
static int wait_for_irq_event(int irq_fd, struct snitch_irq_event *event) {
    struct pollfd pfd;
    ...
    long prc = syscall3(SYS_POLL, (long)&pfd, 1, 5000);
    if (prc < 0) {
        write_all(STDERR_FD, "poll on IRQ device failed\n");
        return 1;
    }
    if (prc == 0) {
        write_all(STDERR_FD, "timeout waiting for Linux IRQ event\n");
        return 1;
    }

    long n = syscall3(SYS_READ, irq_fd, (long)event, sizeof(*event));
    if (n != (long)sizeof(*event)) {
        write_all(STDERR_FD, "read IRQ event failed\n");
        return 1;
    }
...
static int run_one(volatile unsigned char *regs, int irq_fd, const char *tag, u32 arg0, u32 arg1) {
    ...
    if (wait_for_irq_event(irq_fd, &event) != 0) {
        return 1;
    }

    u32 status = read_reg(regs, REG_STATUS);
    ...
    write_hex_join(tag, "IRQ_EVENT_COUNT", event.count);
    write_hex_join(tag, "IRQ_EVENT_STATUS", event.status);
    ...
    write_hex_join(tag, "IRQ_PENDING_AFTER_DRIVER_ACK", irq_pending);
    write_hex32("START_COUNT", start_count);
    write_hex32("RUN_CYCLES ", run_cycles);

    if ((event.status & 0x5u) != 0x5u) {
        write_all(STDERR_FD, "Linux IRQ event did not report pending line\n");
        return 1;
    }

    if ((status & 0x1u) == 0) {
        write_all(STDERR_FD, "done bit not set after IRQ wake\n");
        return 1;
    }
    ...
    if (irq_enable != 1u || (irq_pending & 0x5u) != 0u) {
        write_all(STDERR_FD, "driver did not acknowledge FPGA IRQ line\n");
        return 1;
    }
...
static int run_test(void) {
    ...
    int irq_fd = (int)syscall3(SYS_OPEN, (long)S14_IRQ_DEV, O_RDONLY, 0);
    if (irq_fd < 0) {
        write_all(STDERR_FD, "open /dev/snitch_lite_irq failed; insmod snitch_lite_irq.ko first\n");
        return 1;
    }
    ...
    if (id != 0x53130001u) {
        write_all(STDERR_FD, "unexpected ID; program the SL13 bitstream first\n");
    ...
    if (run_one(regs, irq_fd, "RUN0", 0x00000100u, 0x000000a5u) != 0) {
        syscall3(SYS_MUNMAP, mapped, MAP_LEN, 0);
        syscall1(SYS_CLOSE, mem_fd);
        syscall1(SYS_CLOSE, irq_fd);
        return 1;
    ...
    if (run_one(regs, irq_fd, "RUN1", 0x00000200u, 0x00000055u) != 0) {
        syscall3(SYS_MUNMAP, mapped, MAP_LEN, 0);
        syscall1(SYS_CLOSE, mem_fd);
        syscall1(SYS_CLOSE, irq_fd);
        return 1;
