module safe_host_control #(
  ...
  parameter logic [AvalonAddrWidth-1:0] ClusterBaseWord = 16'h0800
) (
  input  logic                       clk_i,
  input  logic                       rst_i,
  ...
  logic cluster_selected, boot_selected;
  ...
  assign boot_selected = avs_address_i >= BootBaseWord && avs_address_i < ClusterBaseWord;
  ...
  always_ff @(posedge clk_i) begin
    if (rst_i) begin
      ...
      if (avs_write_i && !cluster_selected && !boot_selected && avs_byteenable_i[0]) begin
        case (avs_address_i)
