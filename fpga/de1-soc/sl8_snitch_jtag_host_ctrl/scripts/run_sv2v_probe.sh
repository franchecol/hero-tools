...
"${S8_DIR}/scripts/build_sw_rom.sh"

SV2V_LOG="${GENERATED_DIR}/sv2v_s8.log"
YOSYS_LOG="${GENERATED_DIR}/yosys_s8.log"
OUT_V="${GENERATED_DIR}/snitch_host_ctrl_core.v"
...
SOURCES=(
    ...
    "${FILTERED_SNITCH}"
    "${S8_DIR}/rtl/s8_snitch_host_ctrl_core.sv"
)
...
if [[ "${sv2v_status}" -ne 0 ]]; then
    echo "sv2v SL8 failed with exit code ${sv2v_status}"
...
echo "sv2v SL8 passed"
echo "Output: ${OUT_V}"
echo "Log: ${SV2V_LOG}"

set +e
"${YOSYS}" -q -p "read_verilog ${OUT_V}; hierarchy -top s8_snitch_host_ctrl_core; proc; check" \
    > "${YOSYS_LOG}" 2>&1
yosys_status=$?
set -e

if [[ "${yosys_status}" -ne 0 ]]; then
    echo "yosys SL8 failed with exit code ${yosys_status}"
