...
S7_DIR=$(cd -- "${SCRIPT_DIR}/.." && pwd)
REPO_ROOT=$(git -C "${S7_DIR}" rev-parse --show-toplevel)
...
GENERATED_DIR="${S7_DIR}/generated"
SW_BUILD_DIR="${GENERATED_DIR}/sw"
PROJECT="de1_s7_snitch_tiny_ram_check"
SV2V="${SV2V:-sv2v}"
YOSYS="${YOSYS:-yosys}"
RISCV_PREFIX="${RISCV_PREFIX:-riscv64-unknown-elf}"
S7_SW_SRC="${S7_SW_SRC:-${S7_DIR}/sw/ram_check.S}"
S7_ROM_MAX_WORDS="${S7_ROM_MAX_WORDS:-64}"
...
echo "S7_SW_SRC         = ${S7_SW_SRC}"
echo "S7_ROM_MAX_WORDS  = ${S7_ROM_MAX_WORDS}"
