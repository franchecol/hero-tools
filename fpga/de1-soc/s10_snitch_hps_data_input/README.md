# Snitch-Lite S10: HPS Host Data Input

S10 continues after S9.

S9 proved that ARM Linux can clear/start Snitch-Lite and read its fixed result.
S10 adds the first real host-to-Snitch data path:
...
```bash
cd /home/ftv/builds/hero-tools/fpga/de1-soc/s10_snitch_hps_data_input
./scripts/build.sh
...
Like D3/S9, this project neutralizes the generated HPS SDRAM SDC file because
...
./scripts/serial_transfer.py send \
  ../s10_snitch_hps_data_input/build/s10_mmio_nolibc \
  /tmp/s10_mmio_nolibc
```
