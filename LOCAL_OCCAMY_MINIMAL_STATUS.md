...
M3 HeroSDK-shaped build and runtime smoke proof
  ...
  launch-capture smoke run
    -> extends the fake-completion path with JSONL snapshots
    -> captures 16 OpenMP target launches from offload_benchmark
    -> records each target function address, device argument-buffer address,
       worker count, SoC scratch registers, mailbox-layout words, and
       argument-buffer words from the fake L3 map
  ...
  online replay-bridge smoke run
    -> runs the qemu/Linux HeroSDK OpenMP host and the native Verilator replay
       wrapper in one coordinated smoke
    -> the fake driver captures a launch, writes a bridge request, and blocks
...
- fake-driver `OCCAMY_FAKE_BRIDGE_DIR` replay-bridge support
...
- for the M3 fake-completion smoke, the same log proves that a fake mailbox
  ...
  are still missing
- for the M3 launch-capture smoke, `output/occamy-openmp-smoke/launches.jsonl`
