...
struct l3_layout *l3_layout_v_addr;
uintptr_t l3_layout_p_addr;
...
void hero_dev_reset(HeroDev *dev, unsigned full) {
    ...
    fence();
    // Clean IRQs
    writew(0, car_spatz_cluster + CARFIELD_SNITCH_CLUSTER_CLINT_OFFSET);
    // Reset and de-reset
...
int hero_dev_mmap(HeroDev *dev) {
    ...
    // Put the TCDM map in the local_mems list for OpenMP to use
...
void hero_dev_exe_start(HeroDev *dev) {
	...
	writew(0x78000000, car_spatz_cluster + CARFIELD_SNITCH_CLUSTER_BOOTADR_REG_OFFSET);
    writew((uint32_t)l3_layout_p_addr, car_spatz_cluster + CARFIELD_SNITCH_CLUSTER_SCRATCH_REG_OFFSET);
    fence();

    // Set IRQ for 9 cores (0x1ff)
    writew(0x1ff, car_spatz_cluster + CARFIELD_SNITCH_CLUSTER_CLINT_OFFSET);
}
...
int hero_dev_init(HeroDev *dev) {
    ...
    l3_layout_v_addr = (struct l3_layout *) hero_dev_l2_malloc(dev, sizeof(struct l3_layout), &l3_layout_p_addr);

    l3_layout_v_addr->h2a_mbox = dev->mboxes.h2a_mbox_mem.p_addr;
    l3_layout_v_addr->a2h_mbox = dev->mboxes.a2h_mbox_mem.p_addr;
    writew((uint32_t)l3_layout_p_addr, car_spatz_cluster + CARFIELD_SNITCH_CLUSTER_SCRATCH_REG_OFFSET);
    fence();
    pr_trace("%s wrote %x at %llx\n", __func__, (uint32_t)l3_layout_p_addr, car_spatz_cluster + CARFIELD_SNITCH_CLUSTER_SCRATCH_REG_OFFSET);
    return 0;
}
